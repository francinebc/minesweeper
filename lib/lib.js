...
function setupButtons(){
  ...
  var buttonNames = ["Easy", "Medium", "Hard"];
  var buttons = [];
  for(var i=0; i<buttonNames.length; i++){
    var button = document.createElement("button");
    button.setAttribute("id", buttonNames[i]);
    var text = document.createTextNode(buttonNames[i]);
    button.appendChild(text);
    notes.appendChild(button);
    buttons.push(button);    
  }

  // I tried adding these in a loop but the value passed to gameInit was the last value of i so it would always pass '3'
  buttons[0].addEventListener('click', function(){gameInit(0);});
  buttons[1].addEventListener('click', function(){gameInit(1);});
  buttons[2].addEventListener('click', function(){gameInit(2);});
}
