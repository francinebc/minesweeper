...
function setInitParams(){
  ...
  medButton.setAttribute("id", "medButton");
  var medText = document.createTextNode("Medium");
  medButton.appendChild(medText);
  ...
  // add buttons to DOM
  notes.appendChild(easyButton);
  notes.appendChild(medButton);
  notes.appendChild(hardButton);

  // add listeners for buttons
  easyButton.addEventListener('click', function(){gameInit(0);});
  medButton.addEventListener('click', function(){gameInit(1);});
  hardButton.addEventListener('click', function(){gameInit(2);});
}
